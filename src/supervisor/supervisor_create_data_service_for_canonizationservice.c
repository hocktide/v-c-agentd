/**
 * \file supervisor/supervisor_create_data_service_for_canonizationservice.c
 *
 * \brief Create the data service for the canonization service.
 *
    ...
    const agent_config_t* conf, int* data_socket, int* log_socket)
{
    ...
    /* canonization service can read latest block ID. */
    BITCAP_SET_TRUE(data_proc->reducedcaps,
        DATASERVICE_API_CAP_APP_BLOCK_ID_LATEST_READ);
    /* canonization service can read next block ID. */
    BITCAP_SET_TRUE(data_proc->reducedcaps,
        DATASERVICE_API_CAP_APP_BLOCK_ID_NEXT_READ);
    /* canonization service can read previous block ID. */
    BITCAP_SET_TRUE(data_proc->reducedcaps,
        DATASERVICE_API_CAP_APP_BLOCK_ID_PREV_READ);
    /* canonization service can read the block ID for a transaction. */
    BITCAP_SET_TRUE(data_proc->reducedcaps,
        DATASERVICE_API_CAP_APP_BLOCK_ID_WITH_TRANSACTION_READ);
    /* canonization service can read a block. */
    BITCAP_SET_TRUE(data_proc->reducedcaps,
        DATASERVICE_API_CAP_APP_BLOCK_READ);
    /* canonization service can read a transaction by ID. */
    BITCAP_SET_TRUE(data_proc->reducedcaps,
        DATASERVICE_API_CAP_APP_TRANSACTION_READ);
    /* canonization service can submit a transaction to the process queue. */
    BITCAP_SET_TRUE(data_proc->reducedcaps,
        DATASERVICE_API_CAP_APP_PQ_TRANSACTION_SUBMIT);
    /* canonization service can read the first txn from process queue. */
    BITCAP_SET_TRUE(data_proc->reducedcaps,
        DATASERVICE_API_CAP_APP_PQ_TRANSACTION_FIRST_READ);
    /* canonization service can read a transaction from the process queue. */
    BITCAP_SET_TRUE(data_proc->reducedcaps,
        DATASERVICE_API_CAP_APP_PQ_TRANSACTION_READ);
    /* canonization service can drop a transaction from the process queue. */
    BITCAP_SET_TRUE(data_proc->reducedcaps,
        DATASERVICE_API_CAP_APP_PQ_TRANSACTION_DROP);
    /* canonization service can read an artifact by ID. */
    BITCAP_SET_TRUE(data_proc->reducedcaps,
        DATASERVICE_API_CAP_APP_ARTIFACT_READ);
    /* canonization service can write a block to the block table. */
    BITCAP_SET_TRUE(data_proc->reducedcaps,
        DATASERVICE_API_CAP_APP_BLOCK_WRITE);
    /* canonization service can query a block ID by block height. */
    BITCAP_SET_TRUE(data_proc->reducedcaps,
