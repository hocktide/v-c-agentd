/**
 * \file supervisor/supervisor_create_data_service_for_auth_protocol_service.c
 *
 * \brief Create the data service for the auth protocol service.
 *
    ...
    const agent_config_t* conf, int* data_socket, int* log_socket)
{
    ...
    /* auth protocol service can read latest block ID. */
    BITCAP_SET_TRUE(data_proc->reducedcaps,
        DATASERVICE_API_CAP_APP_BLOCK_ID_LATEST_READ);
    /* auth protocol service can read next block ID. */
    BITCAP_SET_TRUE(data_proc->reducedcaps,
        DATASERVICE_API_CAP_APP_BLOCK_ID_NEXT_READ);
    /* auth protocol service can read previous block ID. */
    BITCAP_SET_TRUE(data_proc->reducedcaps,
        DATASERVICE_API_CAP_APP_BLOCK_ID_PREV_READ);
    /* auth protocol service can read the block ID for a transaction. */
    BITCAP_SET_TRUE(data_proc->reducedcaps,
        DATASERVICE_API_CAP_APP_BLOCK_ID_WITH_TRANSACTION_READ);
    /* auth protocol service can read a block. */
    BITCAP_SET_TRUE(data_proc->reducedcaps,
        DATASERVICE_API_CAP_APP_BLOCK_READ);
    /* auth protocol service can read a transaction by ID. */
    BITCAP_SET_TRUE(data_proc->reducedcaps,
        DATASERVICE_API_CAP_APP_TRANSACTION_READ);
    /* auth protocol service can submit a transaction to the process queue. */
    BITCAP_SET_TRUE(data_proc->reducedcaps,
        DATASERVICE_API_CAP_APP_PQ_TRANSACTION_SUBMIT);
    /* auth protocol service can read an artifact by ID. */
    BITCAP_SET_TRUE(data_proc->reducedcaps,
        DATASERVICE_API_CAP_APP_ARTIFACT_READ);
    /* auth protocol service can query a block ID by block height. */
    BITCAP_SET_TRUE(data_proc->reducedcaps,
