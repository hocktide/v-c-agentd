/**
 * \file consensus/consensus_service_decode_and_dispatch_control_command.c
 *
    ...
    const void* req, size_t size)
{
    ...
    switch (method)
    {
        /* configure the consensus service. */
        case CONSENSUSSERVICE_API_METHOD_CONFIGURE:
            return consensus_service_decode_and_dispatch_control_command_configure(
                instance, sock, breq, payload_size);

        /* start the consensus service. */
        case CONSENSUSSERVICE_API_METHOD_START:
            return consensus_service_decode_and_dispatch_control_command_start(
                instance, sock, breq, payload_size);
        ...
        default:
            ...
            consensus_service_decode_and_dispatch_write_status(
                sock, method, 0U,
                AGENTD_ERROR_CONSENSUSSERVICE_REQUEST_PACKET_BAD, NULL, 0);

            return AGENTD_ERROR_CONSENSUSSERVICE_REQUEST_PACKET_BAD;
    }
}
