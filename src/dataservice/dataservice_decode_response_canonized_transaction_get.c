/**
 * \file dataservice/dataservice_decode_response_canonized_transaction_get.c
 *
 * \brief Decode the response from the canonized_transaction get api method.
 *
    ...
    dataservice_response_canonized_transaction_get_t* dresp)
{
    ...
    /* | Canonized Transaction get response packet.                         | */
    /* | --------------------------------------------------- | ------------ | */
    /* | DATA                                                | SIZE         | */
    /* | --------------------------------------------------- | ------------ | */
    ...
    if (DATASERVICE_API_METHOD_APP_TRANSACTION_READ !=
    ...
    /* copy the block_id. */
    memcpy(dresp->node.block_id, bval + 64, sizeof(dresp->node.block_id));

    /* copy the state. */
    memcpy(
        &dresp->node.net_txn_state, bval + 80,
