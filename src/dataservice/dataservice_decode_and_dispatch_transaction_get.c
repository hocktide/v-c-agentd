/**
 * \file dataservice/dataservice_decode_and_dispatch_transaction_get.c
 *
 * \brief Decode transaction get request and dispatch the call.
 *
    ...
    size_t size)
{
    ...
    /* transaction get request structure. */
    dataservice_request_transaction_get_t dreq;

    /* parse the request payload. */
    retval = dataservice_decode_request_transaction_get(req, size, &dreq);
    ...
    retval =
        dataservice_encode_response_transaction_get(
            ...
            node.artifact_id, node.net_txn_state, txn_bytes, txn_size);
    if (AGENTD_STATUS_SUCCESS != retval)
    {
        goto done;
    }
    ...
    retval =
        dataservice_decode_and_dispatch_write_status(
            sock, DATASERVICE_API_METHOD_APP_PQ_TRANSACTION_READ,
