/**
 * \file dataservice/dataservice_decode_response_block_get.c
 *
 * \brief Decode the response from the block get api method.
 *
    ...
    dataservice_response_block_get_t* dresp)
{
    ...
    if (DATASERVICE_API_METHOD_APP_BLOCK_READ !=
    ...
    memcpy(dresp->node.first_transaction_id, bval + 48,
        sizeof(dresp->node.first_transaction_id));

    /* copy the block height. */
    memcpy(&dresp->node.net_block_height, bval + 64,
        sizeof(dresp->node.net_block_height));

    /* set the size. */
    dresp->node.net_block_cert_size = htonll(dat_size);
