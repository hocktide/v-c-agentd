/**
 * \file dataservice/dataservice_decode_response_root_context_init.c
 *
 * \brief Decode the response from the root context init api method.
 *
    ...
    dataservice_response_root_context_init_t* dresp)
{
    ...
    if (DATASERVICE_API_METHOD_LL_ROOT_CONTEXT_CREATE != dresp->hdr.method_code)
    {
    ...
    dresp->hdr.payload_size = size - response_packet_size;
