...
int main(int argc, char* argv[])
{
    ...
    retval =
        ipc_read_string_block(lhs, &val);
    if (AGENTD_STATUS_SUCCESS != retval)
    {
        MODEL_ASSERT((AGENTD_ERROR_IPC_READ_BLOCK_FAILURE == retval) || (AGENTD_ERROR_IPC_READ_UNEXPECTED_DATA_TYPE == retval) || (AGENTD_ERROR_IPC_READ_UNEXPECTED_DATA_SIZE == retval) || (AGENTD_ERROR_GENERAL_OUT_OF_MEMORY == retval));
        close(lhs);
        close(rhs);
